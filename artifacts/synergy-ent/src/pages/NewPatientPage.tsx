...
export function NewPatientPage() {
  ...
  const patientCards = (content?.cards?.length ? content.cards : cards).map((card, index) => ({
    ...
    title: card.title ?? cards[index]?.title ?? "",
    items: card.items ?? cards[index]?.items ?? [],
  }));
  const officeHours = content?.hours?.length ? content.hours : hours;

  return (
    <SiteLayout>
      <PageHero
        title={content?.heroTitle ?? "New Patient Information"}
        subtitle={content?.heroSubtitle ?? "Everything you need to know before your first visit."}
        breadcrumb={content?.heroBreadcrumb ?? "New Patient"}
      />
      ...
      <section className="bg-[#BBDBED] py-14 px-6 md:px-12">
        <div className="max-w-4xl mx-auto text-center">
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-widest text-[#1D3A5F]/60">{content?.welcomeEyebrow ?? "Welcome"}</span>
          </div>
          <h2 className="text-2xl md:text-3xl font-['Inter'] text-[#1D3A5F] mb-4">
            {content?.heading ?? "We're excited to welcome you as a new patient."}
          </h2>
          <p className="text-[#1D3A5F]/65 font-light leading-relaxed max-w-2xl mx-auto">
            {content?.intro ?? "Please review the information below to prepare for your first visit. If you have any questions before coming in, our team is happy to help."}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12">
          ...
          >
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-[#BBDBED]/70">{content?.hoursEyebrow ?? "Hours of Operation"}</span>
            </div>
            ...
            <p className="mt-4 text-white/40 text-xs font-light">
              {content?.hoursNote ?? "Closed for lunch Monday - Thursday, 12:00pm - 1:00pm"}
            </p>
          </motion.div>
          ...
          >
            <div>
              <div className="flex items-center gap-2 mb-6">
                <div className="w-1.5 h-1.5 rounded-full bg-[#E7FFD9]" />
                <span className="text-xs font-semibold uppercase tracking-widest text-[#BBDBED]/70">{content?.insuranceEyebrow ?? "Insurance"}</span>
              </div>
              <h3 className="text-white text-2xl font-['Inter'] mb-4">
                {content?.insuranceHeading ?? "We work with most insurance plans."}
              </h3>
              <p className="text-white/55 font-light leading-relaxed mb-6">
                {content?.insurance ?? "Synergy ENT & Wellness is an out-of-network practice. We are happy to provide documentation to help you submit claims to your insurance carrier for potential reimbursement. We do not currently accept Medicare."}
              </p>
              <div
                className="rounded-[16px] p-6 flex flex-col gap-3"
                style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)" }}
              >
                <p className="text-[#BBDBED] text-sm font-semibold">{content?.coverageHeading ?? "Questions about coverage?"}</p>
                <p className="text-white/50 text-sm font-light">
                  {content?.coverageText ?? "Call us before your appointment and we'll help you understand your out-of-network benefits and what to expect."}
                </p>
                <ActionLink
                  link={content?.coverageButton ?? { label: wpData().phone ?? "(201) 453-4540", url: `tel:${(wpData().phone ?? "+12014534540").replace(/[^0-9+]/g, "")}` }}
                ...
                >
                  {content?.coverageButton?.label ?? wpData().phone ?? "(201) 453-4540"} <ArrowRight className="w-3.5 h-3.5" />
                </ActionLink>
              </div>
            </div>
          </motion.div>
      ...
      <section className="bg-[#BBDBED] py-20 px-6 md:px-12 text-center">
        <div className="max-w-2xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-['Inter'] text-[#1D3A5F] mb-4">{content?.ctaHeading ?? "Ready to book your first visit?"}</h2>
          <p className="text-[#1D3A5F]/65 mb-8 font-light">
            {content?.ctaText ?? "Dr. Scheid is currently accepting new patients. We look forward to meeting you."}
          </p>
